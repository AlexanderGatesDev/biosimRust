...
def runTest():
    """ Execute the biosim4 binary using the 'subprocess' module
        Supports both C++ (./bin/Release/biosim4) and Rust (cargo run --release --) binaries
        """
    ...
    # Try Rust binary first (cargo run), fall back to C++ binary if needed
    import os
    from pathlib import Path
    ...
    if cargo_toml.exists():
        # Use Rust binary via cargo run
        shellcmd = "cargo run --release -- %s" % relpath
        print("Running the simulation with Rust binary...\n")
    else:
        # Fall back to C++ binary (now in cpp/ directory)
        cpp_binary = project_root / "cpp" / "bin" / "Release" / "biosim4"
        if cpp_binary.exists():
            shellcmd = "./cpp/bin/Release/biosim4 %s" % relpath
        else:
            shellcmd = "./bin/Release/biosim4 %s" % relpath
        print("Running the simulation with C++ binary...\n")
