...
### How biosimrust sources parameters

If no config file is specified, biosimrust uses the default parameters:

-  Rust version: `biosimrust.ini`

When using testapp.py you can specify only the few parameters that differ from the default parameters. See the sample _microtest_ in ./configs/testapp.ini for an example.

However, the caveat of this approach is that, **should one or more default parameter values change in future**, your **test results may deviate from expectation**. Unless defined explicitly in your test configuration, _biosimrust will substitute default parameter values for any test parameter that is missing_.

Consider this when creating and storing a test configuration.

### Configure a new test

1. Copy the default config file to the _configs/_ directory and rename the copy with the unique name of your new test:

For Rust tests:

```bash
cp ../biosimrust.ini ./configs/my_new_test.ini
```
